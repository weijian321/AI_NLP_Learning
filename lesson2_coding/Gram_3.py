...
print 'pair counter中频率最高前20：\n', json.dumps(two_gram_counts.most_common()[:20], encoding='UTF-8', ensure_ascii=False)
get_pair_prob = get_probability_from_counts(two_gram_counts)
print 'pair概率字典频率最高前20：\n', json.dumps(dict(get_pair_prob.items()[:20]), encoding='UTF-8', ensure_ascii=False)

gram_length = 3
thr_gram_counts = Counter(ALL_CHARACTER[i:i + gram_length] for i in range(len(ALL_CHARACTER) - gram_length))
print 'thr counter中频率最高前20：\n', json.dumps(thr_gram_counts.most_common()[:20], encoding='UTF-8', ensure_ascii=False)
get_thr_prob = get_probability_from_counts(thr_gram_counts)
print 'thr 概率字典频率最高前20：\n', json.dumps(dict(get_thr_prob.items()[:20]), encoding='UTF-8', ensure_ascii=False)
...
def get_3_gram_prob(prev2, prev1, word):
    if get_thr_prob.has_key(prev1 + word):
        if get_pair_prob.has_key(prev2 + prev1 + word):
            return get_thr_prob[prev2 + prev1 + word] / get_pair_prob[prev2 + prev1]
        else:
            return get_pair_prob[prev1 + word] / get_char_probability(prev1)
    else:
        return get_char_probability(word)
...
def get_2_gram_string_prob(string):
    ...
    for i, c in enumerate(string):
        ...
        probablities.append(get_2_gram_prob(prev, c))
    return reduce(mul, probablities)


def get_3_gram_string_prob(string):
    probablities = []
    for i, c in enumerate(string):
        prev2 = '' if i == 0 or i == 1 else string[i - 2]
        prev1 = '' if i == 0 else string[i - 1]
        probablities.append(get_3_gram_prob(prev2, prev1, c))
    return reduce(mul, probablities)
...
get_probability_prefromance(get_3_gram_string_prob, pairs)  # prob_of_string
